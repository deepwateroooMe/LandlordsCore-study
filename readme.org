...
#+END_SRC
* 项目的理解消化：
- 前面关于登录注册服与用户帐户管理写入数据库的部分，逻辑已经理解得狠清楚
- 现将斗地主游戏的游戏逻辑，几个按钮的客户端调用，与服务器端的处理逻辑梳理完。为自己下一个可能的拖拉机双升或是打麻将作点儿必要的准备。 
- 今天上午希望把上面想要梳理的逻辑追源码再看一遍看完。 
...
** 点击注册后的日志（5  5）
- 早上还是想要看些更有难度，自已经前不太懂的源码，或是去理解脑袋不清醒时理解不了的概念。【爱表哥，爱生活！！！】
