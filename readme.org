...
- 主要是以牌桌上的：什么地主牌呀，一些清空（选中选中过的牌，就是跳高跳出来准备出的，为什么先前没有处理？），重置，等预处理
- 牌桌，地主牌
    #+BEGIN_SRC csharp
[MessageHandler]
public class Actor_GameStart_NttHandler : AMHandler<Actor_GameStart_Ntt> {
    ...
    #+END_SRC
*** GamerUIComponent: 每个玩家身上所背的UI 小面板，用来显示这个玩家的相关信息
- 包括UI 面板，必要的用户信息，玩家昵称
- 功能包括：抢不抢地主什么的
- LandlordsInteractionComponent: 它说，这个组件更像是出牌互动功能模块？
    #+BEGIN_SRC csharp
// 每个玩家的UI组件
...
** C2G_GetUserInfo_ReqHandler:【网关服】处理用户信息查询请求：它就去拿数据库相关组件。因为 ET7 重构了数据库模块，这里这略过。
    #+BEGIN_SRC csharp
...
* 准备按钮： Prompt-text, 可是它是可点击的按钮 promptButton
** promptButton: OnPrompt() 回调函数
- 从游戏界面上看，这个回调后，本地玩家的手牌显示出来了。地主的三张牌画出背面，不能看
- 并在本地玩家手牌显示出来后，玩家看过自己的手牌，接下来可以决定：是否抢地主
    #+BEGIN_SRC csharp
private async void OnPrompt() { // 提示
    ...
    #+END_SRC
** Actor_GamerPrompt_ReqHandler:【地图服】说，你不是假人，那我就给你分牌，给你手里的牌排好序，然后下发给你【客户端】
    #+BEGIN_SRC csharp
[ActorMessageHandler(AppType.Map)]
public class Actor_GamerPrompt_ReqHandler : AMActorRpcHandler<Gamer, Actor_GamerPrompt_Req, Actor_GamerPrompt_Ack> {
    ...
    #+END_SRC
** HandCardsComponentSystem: 处理玩家手牌的一些相关信息
    #+BEGIN_SRC csharp
public static class HandCardsComponentSystem {
    ...
    #+END_SRC
** DeskCardsCacheComponentSystem: 发牌，添加牌，获取总值等相关操作，逻辑
    #+BEGIN_SRC csharp
...
* 抢地主按钮：抢与不抢
** Actor_GamerGrabLandlordSelect_NttHandler:【地图服】处理 抢地主逻辑。
   #+BEGIN_SRC csharp
[ActorMessageHandler(AppType.Map)]
public class Actor_GamerGrabLandlordSelect_NttHandler : AMActorHandler<Gamer, Actor_GamerGrabLandlordSelect_Ntt> {
   ...
   #+END_SRC
** Actor_AuthorityGrabLandlord_NttHandler
    #+BEGIN_SRC csharp
[MessageHandler]
public class Actor_AuthorityGrabLandlord_NttHandler : AMHandler<Actor_AuthorityGrabLandlord_Ntt> {
    ...
    #+END_SRC
** Actor_SetLandlord_NttHandler: 设置地主
- 亲爱的表哥永远是活宝妹这里最受尊重爱护的亲爱的表哥～～！！任何时候，活宝妹就是一定要、一定会嫁给偶亲爱的表哥！！！
    #+BEGIN_SRC csharp
    #+END_SRC

* 出牌：
** TrusteeshipComponentSystem: 自动抢地主，或是催玩家该出牌了
#+BEGIN_SRC csharp
...
#+END_SRC
** Actor_GamerDontPlay_NttHandler:【网关服】处理，当前本地玩家，不出牌的情况。就是接下来大概代为打牌，或是亲爱的表哥出面会话的情况。
- 这里，还没去读，没弄明白，什么情况下，出现或是算作，当前本地玩家不出牌？找到一个出处在上面。机器人，没地主时，会自动帮抢一个地主；或是催玩家出牌，或是提示可以出什么牌。
- 感觉再看这部分的逻辑，还是需要自己的项目能够跟上，在Unity 可以测试通前几个界面： UILobby, TractorRoom 之后，再来开发游戏界面与游戏逻辑，理解得更透彻，也会开发得效率稍微高一点儿。今天下午去弄这些上午弄明白的。
#+BEGIN_SRC csharp
[ActorMessageHandler(AppType.Map)]
...
* HandCardSprite: 想看看这个类到底是什么原理 
- ETModel 客户端里的。
- 不知道哪个预设或是控件添加了这个脚本？我的斗地主参考项目客户端目前运行不到打牌的地方（感觉这个类应该是添加在玩家手牌相关的地方）就先抛错了
- 今天下午还看不动这个，就去梳理游戏逻辑
...
- 把这些慢慢看得差不多，就可以试着开始想：拖拉机要怎么才能设计成这川可以与服务器交互的多人网络游戏呢？【任何时候，活宝妹就是一定要、一定会嫁给偶亲爱的表哥！！！】
